...
export async function audit(action: string, req: AuthedRequest, opts: { targetType?: string; targetId?: string; before?: any; after?: any; actorType?: string; actorId?: string } = {}): Promise<void> {
  try {
    const actorType = opts.actorType ?? (String(req.userId ?? '').startsWith('doc') ? 'DOCTOR' : 'ADMIN');
    await prisma.auditLog.create({
      data: {
        actorType,
        // actorId explícito cobre o LOGIN (req.userId ainda não existe na rota pública).
        actorId: opts.actorId ?? req.userId ?? null,
        action,
