...
export const RISK_RULES: RiskRules = {
    ...
    },
    {
      ...
      bands: [
        { max: 49, severity: 'moderate', condition: 'cardiovascular_risk',
          finding: 'HDL abaixo da faixa para mulher (<50). Quando associado a outros fatores (obesidade, hipertensão, diabetes, tabagismo), pode contribuir para o aumento do risco cardiovascular' },
      ],
      bandsMale: [
        { max: 39, severity: 'moderate', condition: 'cardiovascular_risk',
          finding: 'HDL abaixo da faixa de referência (<40). Quando associado a outros fatores (obesidade, hipertensão, diabetes, tabagismo), pode contribuir para o aumento do risco cardiovascular' },
      ],
    },
    {
    ...
    // ---- ÍNDICES DERIVADOS (M2) — calculados, não extraídos do laudo ----
    {
      ...
      bands: [
        ...
        { min: 25, max: 29.9, severity: 'low', condition: 'obesidade',
          finding: 'IMC na faixa de sobrepeso (25–29,9). O IMC é útil como triagem, mas não diferencia massa muscular de gordura corporal — associado a aumento do risco cardiovascular e metabólico quando combinado com outros fatores.' },
        { min: 30, max: 34.9, severity: 'moderate', condition: 'obesidade',
          finding: 'IMC na faixa de obesidade grau I (30–34,9). O IMC não diferencia massa muscular de gordura corporal — associado a risco aumentado de diabetes e hipertensão quando combinado com outros fatores.' },
  ...
  narratives: {
    ...
    cardiovascular_risk: 'foram identificados fatores associados ao aumento do risco cardiovascular (como HDL abaixo da faixa e/ou IMC elevado). Esses achados devem ser interpretados em conjunto com idade, pressão arterial, diabetes, tabagismo, histórico familiar e outros exames',
  ...
  conditionLabel: {
    ...
    cardiovascular_risk: 'Fatores de risco cardiovascular',
  ...
  followUpTests: {
    ...
    cardiovascular_risk: ['Perfil lipídico', 'PCR ultrassensível', 'Eletrocardiograma', 'Circunferência abdominal', 'Controle de pressão arterial', 'Glicemia + HbA1c (se não houver recente)'],
