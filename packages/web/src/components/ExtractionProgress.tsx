...
/** Progresso animado com o Dr. Exame durante a extração — passa credibilidade. */
export const ExtractionProgress = () => {
  const [step, setStep] = useState(0);
  const [pct, setPct] = useState(8);
  useEffect(() => {
    const stepTimer = setInterval(() => setStep((s) => Math.min(s + 1, STEPS.length - 1)), 4500);
    // sobe rápido até ~80, depois avança devagar (sem travar num número) — a IA leva tempo
    const pctTimer = setInterval(() => setPct((p) => (p < 80 ? p + 4 : Math.min(p + 0.4, 97))), 700);
    ...
    <Box sx={{ textAlign: 'center', py: 4, px: 2, mt: 2, borderRadius: 4, background: 'linear-gradient(135deg,#e6f7f6,#eef7f6)', border: '1px solid #cfe9e8' }}>
      ...
      <Box sx={{ maxWidth: 380, mx: 'auto' }}>
        <LinearProgress variant="determinate" value={pct}
          sx={{ height: 12, borderRadius: 6, bgcolor: '#d6efed', '& .MuiLinearProgress-bar': { borderRadius: 6, background: 'linear-gradient(90deg,#20b2aa,#10b981)' } }} />
