...
export const ReportSectionCard = ({ icon, title, accent, count, children }: {
  ...
  <Card sx={{ overflow: 'hidden', background: `${accent}0a`, border: '1px solid', borderColor: `${accent}33`, borderLeft: `4px solid ${accent}` }}>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, px: 2, py: 1.5 }}>
      <Box sx={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: `${accent}1f`, color: accent, '& svg': { fontSize: 20 } }}>{icon}</Box>
      <Typography sx={{ fontWeight: 800, fontFamily: '"Poppins",sans-serif', flex: 1, fontSize: '1.02rem', minWidth: 0 }}>{title}</Typography>
      {count != null && <Chip size="small" label={count} sx={{ bgcolor: `${accent}22`, color: accent, fontWeight: 700, height: 22, flexShrink: 0 }} />}
    </Box>
