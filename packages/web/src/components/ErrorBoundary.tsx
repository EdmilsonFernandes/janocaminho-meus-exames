    ...
    if (this.state.hasError) {
      return (
        <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 3, background: 'background.default', textAlign: 'center' }}>
          ...
          <Button variant="contained" size="large" onClick={() => this.setState({ hasError: false, error: undefined })} sx={{ borderRadius: '999px', textTransform: 'none', fontWeight: 800, px: 4, background: 'linear-gradient(180deg,#20b2aa,#178f89)', '&:hover': { background: 'linear-gradient(180deg,#178f89,#178f89)' } }}>Tentar novamente</Button>
        </Box>
      );
    }
