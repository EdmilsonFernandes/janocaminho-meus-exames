          ...
          const card = (q: any) => {
            ...
            return (
              <AppCard key={q.id} sx={{ mb: 1.5, border: '1px solid', borderColor: answered ? 'divider' : 'transparent', borderRadius: '20px' }}><CardContent sx={{ p: { xs: 1.75, md: 2 } }}>
                ...
                {lastPatient && <Typography variant="body2" sx={{ mt: 1.25, color: 'text.secondary', fontStyle: 'italic', pl: 0.5 }}>"{String(lastPatient.body).slice(0, 160)}{(String(lastPatient.body).length ?? 0) > 160 ? '…' : ''}"</Typography>}
                {/* Resposta INLINE + respostas prontas */}
                {isOpen && !answered && (
                  <Box sx={{ mt: 1.5 }}>
