...
export const ChatPage = () => {
  ...
  return (
    <Box sx={{ maxWidth: 820, mx: 'auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 110px)', p: { xs: 1, md: 2 } }}>
      ...
      {/* mensagens */}
      <Box ref={scrollRef} sx={{ flex: 1, minHeight: 0, overflowY: 'auto', p: 1, background: '#f1f6f5', borderRadius: 3 }}>
        {messages.length === 0 && (
            ...
            return (
              <Box key={i} sx={{ display: 'flex', gap: 0.75, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', alignItems: 'flex-end' }}>
                ...
                <Paper elevation={0} sx={{
                  ...
                  bgcolor: m.role === 'user' ? TEAL : '#ffffff',
      ...
      <Box component="form" onSubmit={(e: any) => { e.preventDefault(); send(); }} sx={{ display: 'flex', gap: 0.75, alignItems: 'center', mt: 1, p: 0.5, pl: 1, borderRadius: 99, border: '1px solid #bfe0dc', bgcolor: '#fff', boxShadow: '0 2px 12px rgba(32,178,170,.08)' }}>
      ...
      <Drawer anchor="right" open={histOpen} onClose={() => setHistOpen(false)} PaperProps={{ sx: { width: { xs: '100%', sm: 370 } } }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, borderBottom: '1px solid #e2efec', background: 'linear-gradient(135deg,#20b2aa,#178f89)', color: '#fff' }}>
        ...
        <Box sx={{ overflowY: 'auto', flex: 1 }}>
          ...
          {BUCKETS.map((bk) => {
            ...
            return (
              <Box key={bk}>
                <Typography sx={{ fontSize: 11, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5, px: 2, pt: 1.5, pb: 0.5 }}>{bk}</Typography>
...
const HistoryRow = ({ conv, active, onOpen, onRename, onDelete }: { conv: Conv; active: boolean; onOpen: () => void; onRename: (t: string) => void; onDelete: () => void }) => {
  ...
  return (
    <ListItemButton selected={active} onClick={onOpen} sx={{ py: 1, px: 2, '&.Mui-selected': { bgcolor: 'rgba(32,178,170,.08)' }, '&.Mui-selected:hover': { bgcolor: 'rgba(32,178,170,.12)' } }}>
      <ListItemIcon sx={{ minWidth: 34 }}><ChatBubbleIcon sx={{ color: active ? TEAL : '#94a3b8', fontSize: 18 }} /></ListItemIcon>
      <ListItemText primary={conv.title || 'Sem título'} primaryTypographyProps={{ fontSize: 13.5, fontWeight: active ? 700 : 500, color: '#0f3d3a', noWrap: true }} secondary={new Date(conv.updatedAt).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} secondaryTypographyProps={{ fontSize: 11 }} />
      <IconButton size="small" edge="end" onClick={(e) => { e.stopPropagation(); setMenu(e.currentTarget); }}><MoreVertIcon fontSize="small" sx={{ color: '#94a3b8' }} /></IconButton>
